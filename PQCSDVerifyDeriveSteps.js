...
value is produced as output containing the following fields: "signature", "proofHash",
"salts",, "saltedHashes", "nonMandatory"?, and "mandatoryHash".
*/
...
as output, which contains a set to five elements, using the names "signature", "salts",
"saltedHashes", "labelMap", and "mandatoryIndexes".
...
let [signature, salts, saltedHashes, labelMapCompressed, mandatoryIndexes, selectiveIndexes] = decodeThing
...
writeFile(baseDir + 'verifyCreateVerifyData.json', JSON.stringify(createVerifyData, null, 2))
...
const toVerify = concatBytes(proofHash, mandatoryHash, ...salts, ...saltedHashes);
...
let verificationResult = ml_dsa44.verify(signature, msgHash, pbk)
...
nonMandatory.forEach((value, index) => {
  let aSaltedHash = sha256(concatBytes(salts[selectiveIndexes[index]], utf8encoder.encode(value)));
